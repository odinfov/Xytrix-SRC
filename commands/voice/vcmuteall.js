...
module.exports = {
    ...
    category: 'voice',
    aliases: [],
    description: `Mutes all the users in the voice channel.`,
    premium: false,
    run: async (client, message, args) => {
        ...
        if (!admin && !isSpecialMember && !message.member.permissions.has('MUTE_MEMBERS')) {
        ...
        if (!admin && !own && message.guild.me.roles.highest.position >= message.member.roles.highest.position) {
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor(client.color)
