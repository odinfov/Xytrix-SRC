...
module.exports = {
    ...
    subcommand: ['status'],
    category: 'mod',
    description: `Assigns a specified role to all members in the server.`,
    premium: false,
    ...
    run: async (client, message, args) => {
        ...
        if (args[0] === 'status') {
            ...
            const membersWithRole = message.guild.members.cache.filter(member => member.roles.cache.has(roleAssignmentInProgress.role.id)).size;
            ...
            if (roleToAdd.permissions.has(permission)) {
                return message.channel.send({
                    embeds: [
                        embed
                            .setColor(client.color)
                            .setDescription(`${client.emoji.cross} This Role Is Having Dangerous Permission. I cannot assign that role to all the members.`)
                    ]
                });
            }
        }
...
async function handleRateLimit(client, embed, interactionMessage) {
    embed.setDescription(`An error occurred while giving roles to all members.`);
