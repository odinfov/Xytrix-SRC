...
module.exports = {
    ...
    description: `Ignore specific channels`,
    premium: true,
    run: async (client, message, args) => {
        ...
        if (!isSpecialMember && !hasHigherRole && message.author.id !== message.guild.ownerId) {
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor(client.color)
                        .setDescription(
                            `${client.emoji.cross} You must be the server owner or have a role higher than the bot to use this command.`
                        )
                ]
            });
        }
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setFooter({ text: `${client.user.username} Is Love`, iconURL: client.user.displayAvatarURL({ dynamic: true }) })
