...
module.exports = {
    ...
    subcommand: ['status'],
    category: 'mod',
    description: `Assigns a role to all human members in the server or checks the status of role assignment.`,
    premium: true,
    ...
    run: async (client, message, args) => {
        ...
        if (args[0] === 'status') {
            ...
            const membersWithRole = message.guild.members.cache.filter(member => !member.user.bot && member.roles.cache.has(roleAssignmentInProgress.role.id)).size;
            const percentageWithRole = ((membersWithRole / totalMembers) * 100).toFixed(2);
            const estimatedTime = Math.ceil((totalMembers - membersWithRole) * 0.4); 
            ...
            return message.channel.send({
                embeds: [
                    embed
                        ...
                        .addField('Percentage of Members:', `${percentageWithRole}%`, false)
            ...
            if (roleToAdd.permissions.has(permission)) {
                return message.channel.send({
                    embeds: [
                        embed
                            .setColor(client.color)
                            .setDescription(`${client.emoji.cross} This Role Is Having Dangerous Permission. I cannot assign that role to humans.`)
                    ]
                });
            }
        }
        ...
        await message.guild.members.fetch(); 
        const totalMembers = message.guild.members.cache.filter(member => !member.user.bot).size;
        const estimatedTime = (totalMembers * 400) / 2;
        ...
        const interactionMessage = await message.channel.send({
            embeds: [
                embed.setDescription(`Do you want to start adding roles to all members?`)
            ], components: [row],
        });
            ...
            if (interaction.isButton()) {
                if (interaction.customId === 'yes_button') {
                    ...
                    await interaction.update({
                        embeds: [
                            embed.setDescription(`${client.emoji.tick} Successfully started adding <@&${roleToAdd.id}> to all humans.`)
                        ],
                        components: [],
                    });
                    ...
                    await message.channel.send({
                        embeds: [
                            embed.setDescription(`${client.emoji.tick} Successfully Given Role To All Members.`)
...
async function handleRateLimit(client, embed, interactionMessage) {
    embed.setDescription(`An error occurred while giving roles to members.`);
