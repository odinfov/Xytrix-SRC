...
module.exports = {
    name: "roast",
    aliases: ["Roast", "burn"],
    description: "Roast a user with a funny insult from an API.",
    category: "fun",
    cooldown: 5,
    premium: false,
    ...
    run: async (client, message, args) => {
        try {
            ...
            const roast = response.data.insult || "Sorry, the roast machine is broken right now.";

            const embed = new MessageEmbed()
                .setTitle("Roast")
                .setDescription(`<@${user.id}>: ${roast}`)
            ...
            const embed = new MessageEmbed()
                .setTitle("Roast (Fallback)")
                .setDescription(`<@${user.id}>: ${fallbackRoast}`)
                .setColor(client.color)
