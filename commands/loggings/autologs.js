...
module.exports = {
    ...
    cooldown: 5,
    category: 'logging',
    description: `Automatically set logs server events or actions.`,
    premium: false,
    run: async (client, message, args) => {
        ...
        if (!isSpecialMember && !client.util.hasHigher(message.member)) {
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor(client.color)
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setFooter({ text: `Note : If you wannna setup logging again use ${message.guild.prefix}logsreset & delete all existinglog channels of Xytrix` })
                ]
            });
        } 

        try {
            let category = message.guild.channels.cache.find(c => c.type === 'GUILD_CATEGORY' && c.name === 'Xytrix Logs');
            if (!category) {
                category = await message.guild.channels.create('Xytrix Logs', {
                    type: 'GUILD_CATEGORY',
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setDescription('All necessary logging channels have been successfully created under the "Xytrix LOGS" category.')
