...
module.exports = class Util {
    ...
    async sendPreview(settings, member) {
        if (!settings.welcome?.enabled)
            return 'Welcome message not enabled in this server. Use `welcome on` to enable it'
    ...
    async setContent(settings, content) {
        settings.welcome.content = content;
        await settings.save();
        return 'Configuration saved! Welcome message content updated';
    }
    ...
    async setAuthor(settings, status) {
        const isEnabled = status.toUpperCase() === 'ON';
        settings.welcome.embed.author = isEnabled;
        await settings.save();
        return 'Configuration saved! Welcome message updated';
    }
    
    async setFooter(settings, text, iconURL) {
        settings.welcome.embed.footer = {
            text: text,
            iconURL: iconURL
        };
        await settings.save();
    ...
    async setMainImage(settings, imageUrl) {
        settings.welcome.embed.image = imageUrl;
        await settings.save();
        return 'Configuration saved! Welcome message updated';        
    ...
    async buildGreeting(member, type, config) {
        ...
        const defaultContent = `<@${member.user.id}>`;
        const defaultDescription = `Hey ${member.displayName}, Welcome to the server.`;

        if (config.content && !config.embed?.description && !config.embed?.title && 
            !config.embed?.author && !config.embed?.footer && !config.embed?.thumbnail && 
            !config.embed?.image) {
            const parsedContent = await this.client.util.parse(config.content, member);
            return { content: parsedContent };
        }

        if (config.embed?.description || config.embed?.title || config.embed?.author || 
            config.embed?.footer || config.embed?.thumbnail || config.embed?.image) {
            ...
            embed.setColor(config.embed?.color || member.client.color);
            ...
            if (config.embed.footer) {
                let footerText = await this.client.util.parse(config.embed.footer.text, member);
                footerText = footerText.replace('{{SPECIAL_TIMESTAMP_MARKER}}', '');
                embed.setFooter({
                    text: footerText,
                    iconURL: config.embed.footer.iconURL || member.guild.iconURL({ dynamic: true })
                });
            }
            ...
            if (
                (config.embed.description && config.embed.description.includes('{timestamp}')) ||
                (config.embed.footer?.text && config.embed.footer.text.includes('{timestamp}'))
            ) {
                embed.setTimestamp();
            }
    ...
    async parse(content, member) {
        ...
        return content
            ...
            .replaceAll(/{timestamp}/g, '{{SPECIAL_TIMESTAMP_MARKER}}')
            .replaceAll(
                /{member:created_at}/g,
        ...
        try {
            ...
            if (funny === 'first') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:Xytrix:1242115139453452308> Antinuke \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'second') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:amod:1242115073007288430> Moderation \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'third') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:autility:1242115098487816252> Utility \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'fourth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:agreet:1242115019509071964> Welcomer \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'fifth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:avc:1242115121296310272> Voice \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'six') {
                ...
                embed.addField(
                    `**<:acustom:1242114993369907230> Customrole \`[${cmd.length}]\`**`,
                    cmd.sort().join(', ')
                )
                await interaction
                    .update({
            ...
            if (funny === 'seven') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:alogg:1242115047086624921> Logging \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'ninth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:alogg:1242115047086624921> Ticket \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'tenth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:alogg:1242115047086624921> Giveaway \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'eleventh') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:alogg:1242115047086624921> Autorespond \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'twelth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:alogg:1242115047086624921> Fun \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'thirteen') {
                _commands = this.client.commands
                    .filter((x) => x.category && x.category === 'selfrole')
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:alogg:1242115047086624921> Selfrole \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'fourteen') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:alogg:1242115047086624921> Ai \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
    ...
    async manageAfk(message, client) {
        ...
        // Code to handle mentioning AFK members
        if (memberMentioned) {
            ...
            if (data) {
                ...
                memberMentioned.send({
                    embeds: [
                        new MessageEmbed()
                            .setColor(client.color)
                            .setTitle(`You were mentioned in ${message.guild.name} by ${message.author.tag}`)
                            .setDescription(`[Jump Url](https://discord.com/channels/${message.guild.id}/${message.channel.id}/${message.id})`)
                            .addFields([
                                {
                                    name: `Content:`,
    ...
    async BlacklistCheck(guild) {
        try {
            if (!guild) {
                console.log('Warning: Guild object is null or undefined');
                return false;
            }
